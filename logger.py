...
class Logger(object):
    ...
    def add_result(self, run, result):
        ...
        self.results[run].append(result)# 原来的


    def print_statistics(self, run=None, mode='max_acc'):# 修改的
        if run is not None:
            ...
            padded_results = [
                r + [[0.0] * len(r[0])] * (max_len - len(r)) if len(r) < max_len else r
                for r in self.results
            ]
            result = 100 * torch.tensor(padded_results)
